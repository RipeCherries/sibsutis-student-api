const loginUser = {
  tags: ['User'],
  description: 'Аутентификация пользователя на основе предоставленных учетных данных.',
  requestBody: {
    required: true,
    content: {
      'application/json': {
        schema: {
          type: 'object',
          properties: {
            userEmail: { type: 'string', format: 'email', description: 'Адрес электронной почты пользователя.' },
            userPassword: { type: 'string', description: 'Пароль пользователя.' },
          },
        ...
        examples: {
          Пример: {
            value: {
              userEmail: 'ivanivanov@example.ru',
  ...
  responses: {
    200: {
      description: 'Данные пользователя, а также access и refresh токены.',
      content: {
        'application/json': {
          schema: {
            type: 'object',
